...
PARAM <- list()
PARAM$experimento <- "clu-DBScan-02"
...
# determino el valor optimo de eps
dbscan::kNNdistplot(dchico[, campos_cluster_esc, with=FALSE], 
  k =  5)
...
# la llamada  a DBScan
Dbscan_cl <- fpc::dbscan(dchico[, campos_cluster_esc, with=FALSE],
  eps= 0.5, 
  MinPts = kclusters ) 
...
dataset[ , cluster := paste0("cluster_", Dbscan_cl$cluster) ]
tb_clusters_qty = dataset[ , list("qty"=.N), cluster ]
tb_clusters_qty

clusters_razonables <- tb_clusters_qty[ qty > 100, cluster]
...
for( i in 1:(n-1) ){
  ...
  grafico <- ggplot( dataset[azar< muestra & cluster %in% clusters_razonables],
...
for( campo in campos_totales ) {
  ...
  grafico <- ggplot( dhistoria[periodo >= -18 & cluster %in% clusters_razonables],
